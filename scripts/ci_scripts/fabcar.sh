...
logs() {
    LOG_DIRECTORY=$WORKSPACE/traceic/$1
...
for LANGUAGE in ${LANGUAGES}; do
    ...
    if [ -d ${LANGUAGE} ]; then
        ...
        if [ ${LANGUAGE} = "javascript" -o ${LANGUAGE} = "typescript" ]; then
            ...
            copy_logs $? traceic-${LANGUAGE}-enrollAdmin
            ${COMMAND} ${PREFIX}registerUser${SUFFIX}
            copy_logs $? traceic-${LANGUAGE}-registerUser
            ${COMMAND} ${PREFIX}query${SUFFIX}
            copy_logs $? traceic-${LANGUAGE}-query
            ${COMMAND} ${PREFIX}invoke${SUFFIX}
            copy_logs $? traceic-${LANGUAGE}-invoke
    ...
    echo -e "\033[32m finished traceic test (${LANGUAGE})" "\033[0m"
done
